...
function UpdateTypeModal({ isOpen, onResolve, onReject,type }) {

  const defaultValues = {nom: type?.nom};
...
const onUpdate = data => {
    onResolve({_id:type?._id,...data});
  };

  return (
    <>
     <Dialog header="Modifier un type de capteur" visible={isOpen} onHide={() => onReject(false)} className="w-1/2">
...
export default create(UpdateTypeModal)