...
function CreateNoeudModal({ isOpen, onResolve, onReject }) {
  ...
  const [Noeuds,setNoeuds] = useState([])
    ...
    const defaultValues = {nom: '', noeud:'', type: ''};
  ...
  const onCreate = data => {
    const {noeud,type} = data;
      onResolve({...data, noeud: noeud.value, type: type.value});
    };
  
  return (
    <>
        <Dialog header="Création de Capteur" visible={isOpen} onHide={() => onReject(false)} className="w-1/2">
